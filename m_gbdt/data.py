...
class DataSet:
    ...
    def __init__(self, filename):  # just for csv data format
        line_cnt = 0
        self.instances = dict()
        self.distinct_valueset = dict()  # just for real value type，只处理实数数据
        for line in open(filename):
            if line == "\n":
                continue
            fields = line[:-1].split(",")
            if line_cnt == 0:  # csv header
                ...
                if line_cnt == 1:  # determine the value type /// distinct_value储存type为实数且不重复的元素
    ...
    #分割数据集为训练集和测试集
    def split_dataset(self,filename,train_name,test_name):
    	contents = open(filename).readlines()
    	count=len(contents)
    	split_size=2*count/3
    	print ("The size of train data is ",split_size)
    	train_file=open(train_name,'w')
    	train_file.writelines(contents[:split_size])
    	train_file.close()
    	test_file=open(test_name,'w')
    	test_file.writelines(contents[:1])
    	test_file.writelines(contents[split_size:])
    	test_file.close()
